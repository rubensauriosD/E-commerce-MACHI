...
passport.use(
  ...
  new localStrategy(
    ...
    (req, email, contrasenia, done) => {
      ...
      const ComparadorDeEncriptado = function (
        contraseniaEnviada,
        contraseniaEnBaseDeDatos
      ) {
      ...
      Usuario.findOne({ where: { email } }).then((resultadoUsuario) => {
        ...
        if (!resultadoUsuario) {
          ...
          ComparadorDeEncriptado(contrasenia, resultadoUsuario.contrasenia)
        ) {
